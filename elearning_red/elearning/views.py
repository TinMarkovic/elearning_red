...
def course_show(request, course_id=None):
	...
	rating = M.Rating.objects.filter(user=request.user).filter(course=course_id)
        ...
        if len(rating) == 0:
	    rating = M.Rating(user=M.CustomUser.objects.get(id=request.user.id), course=M.Course.objects.get(id=course_id))
	    rating.value = 0
	    rating.save()
        else:	
	    rating = get_object_or_404(M.Rating, user=M.CustomUser.objects.get(id=request.user.id), course=M.Course.objects.get(id=course_id))
